...
export default function TermsPage() {
  ...
  const sections = [
    ...
    {
      icon: <RotateCcw className="w-5 h-5 text-pink-600" />,
      title: "Rental Guidelines",
      desc: "Rental agreements listed inside Temporary Rentals must clearly specify either refundable cash deposits or physical student ID collaterals. Renters must return assets on time and in their original state. Late fees or damage assessments must be resolved directly between students in a fair manner."
    },
    {
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#090D16] text-[#475569] dark:text-zinc-300 flex flex-col justify-between font-sans transition-colors duration-200">
      ...
      <main className="w-full max-w-4xl mx-auto px-6 py-12 md:py-16 z-10 relative flex-grow">
        ...
        <div className="space-y-4 mb-12">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-100 text-xs font-bold tracking-wide text-indigo-600">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
          {sections.map((sec, idx) => (
            <div 
              key={idx}
              className="bg-white dark:bg-zinc-950 border border-[#E2E8F0] dark:border-zinc-900 rounded-2xl p-6 md:p-8 space-y-4 shadow-sm flex flex-col justify-between"
            >
              <div className="space-y-3">
                <div className="p-2.5 bg-zinc-50 dark:bg-zinc-900 rounded-xl w-fit">
                  {sec.icon}
                </div>
                <h3 className="font-bold text-base text-[#0F172A] dark:text-white">{sec.title}</h3>
        ...
        {/* Warning Policy Box */}
        <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/50 rounded-2xl p-6 space-y-2">
          <h4 className="text-sm font-bold text-red-700 dark:text-red-400 uppercase tracking-wide flex items-center gap-1.5">
            <ShieldAlert className="w-4 h-4 text-red-600" /> Zero-Tolerance Scam Alert
          </h4>
          <p className="text-sm text-red-800 dark:text-zinc-300 font-semibold leading-relaxed">
